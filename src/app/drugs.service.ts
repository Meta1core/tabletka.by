...
export class DrugsService {
  ...
  //   } 
  getRegions(): Observable<any> {
    return this.http.get("http://localhost:8080/drugs/regions");
  } 
  getDrugs(drugName : string, region: number): Observable<any> {
    ...
    return this.http.get("http://localhost:8080/drugs/search", {params: params});
    }
    else {
      let headers = new HttpHeaders();
      headers.append('Content-Type', 'application/json');
  ...
  findPharmacy(ls_num: string, region: number): Observable<any> {
    ...
    return this.http.get("http://localhost:8080/drugs/pharmacy", {params: params});
    }
    else {
      let headers = new HttpHeaders();
      headers.append('Content-Type', 'application/json');
  ...
  aboutPharmacy(name:string, ls_num:number, region:number): Observable<any> {
    ...
    return this.http.get("http://localhost:8080/drugs/pharmacy/about", {params: params});
  }
